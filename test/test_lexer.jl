@testsnippet LexerHelpers begin
    using Waiij
    ...
    function check_tokens(input::String, expected_tokens::Vector{Token})
        l = Lexer(input)
        for tt in expected_tokens
            tok = next_token!(l)
            @test tt.type == tok.type
            @test tt.literal == tok.literal
        end
    end
end

@testitem "Lexer: let statements" setup=[LexerHelpers] begin
...
@testitem "Lexer: function literal" setup=[LexerHelpers] begin
...
@testitem "Lexer: function call" setup=[LexerHelpers] begin
...
@testitem "Lexer: prefix operators" setup=[LexerHelpers] begin
    check_tokens("!-/*5;", [T(BANG), T(MINUS), T(SLASH), T(ASTERIK), Token(INT, "5"), T(SEMICOLON), T(EOF)])
end

@testitem "Lexer: comparison operators" setup=[LexerHelpers] begin
    check_tokens("5 < 10 > 5;", [Token(INT, "5"), T(LT), Token(INT, "10"), T(GT), Token(INT, "5"), T(SEMICOLON), T(EOF)])
end

@testitem "Lexer: if/else statement" setup=[LexerHelpers] begin
...
@testitem "Lexer: equality operators" setup=[LexerHelpers] begin
